...
class PolicySAC(nn.Module):
    ...
    def sample(self, state, zone_box, agent):
        ...
        if zone_box:
            # print("decrease velocitys")
            self.max_action = torch.FloatTensor([0.7, 0.4]).to(self.dev)
            self.min_action = torch.FloatTensor([-0.7, 0]).to(self.dev)
        else: 
            self.max_action = torch.FloatTensor([0.5, 0.4]).to(self.dev)
            self.min_action = torch.FloatTensor([-0.5, 0.0]).to(self.dev)
        self.action_scale = (self.max_action - self.min_action) / 2.0
...
class MaplessNavigatorServer(Node):
    ...
    def getState(self, scan, done):
        ...
        # obstacle_angle = np.argmin(scan.ranges)
        #print("item laser " + str(item))

                        
        if obstacle_min_range < min_range: 
            done = True
            # print("obstale")
            # self.move_base(-0.05, 0.0)
        ...
        constant = (6.0-min_range)*np.ones(105)
    ...
    def reset(self, done):
        ...
        # self.goal_x, self.goal_y = 0.27,6.26
        self.goal_x, self.goal_y = 8.0, 9.0
        # time.sleep(0.1)
    ...
    def run(self,done,total_trial,arrival, collision):
        ...
        if done:
            ...
            if arrv:
                ...
                # print("Xoay robot den huong tiep theo")
            else:
                self.collision   += 1
                # print("Current Trial: ",total_trial)         
            self.total_trial+=1 
