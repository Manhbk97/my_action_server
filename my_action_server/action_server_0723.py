...
class PolicySAC(nn.Module):
    def __init__(self, state_dim, action_dim, actor_lr, DEVICE):
        ...
        self.LOG_STD_MIN = -20
        self.LOG_STD_MAX = 2
        # self.max_action = torch.FloatTensor([1.0, 0.5]).to(self.dev)
        # self.min_action = torch.FloatTensor([-1.0, 0.0]).to(self.dev)
        # self.action_scale = (self.max_action - self.min_action) / 2.0
        # self.action_bias  = (self.max_action + self.min_action) / 2.0
...
class SAC_Agent:
    def __init__(self):    
        ...
        self.SAC_pi.load_state_dict(torch.load("/home/ras/nana_driver/src/mapless_navigation_ros2/saved_model/sacmay_05_0.8_6_nana_pretrain_EP370.pt", map_location=self.DEVICE))
...
class DSAC_Agent:
    def __init__(self):    
        ...
        self.DSAC_pi = PolicyDSAC(24, 2, 0.0001, self.DEVICE)
        ...
        # self.DSAC_pi.load_state_dict(torch.load("/home/ras/nana_driver/src/mapless_navigation_ros2/mapless_navigation_ros2/dsac/model/sac21_input_DSAC_nana_RAstech_env_0723_EP370.pt", map_location=self.DEVICE))
        self.DSAC_pi.load_state_dict(torch.load("/home/ras/nana_driver/src/mapless_navigation_ros2/mapless_navigation_ros2/dsac/model/sac21_input_DSAC_nana_RAstech_env_0723_EP280.pt", map_location=self.DEVICE))
...
class MaplessNavigatorServer(Node):
    ...
    def getOdometry(self, odom):
        ...
        self._moved_distance.data += self.calculate_distance(NewPosition, self.position)
    ...
    def getState(self, scan, done):
        ...
        for i, item in enumerate(filter_lidar):
            ...
            if (i%mod==0 and i!= 104 and i!= 110 and i!= 100 and i!= 80  and i!=60 and i!= 66 and i!= 120 and i!=130 and i!= 140 and i!=150):  #item !=0 and 
                if item==0 or item>= 6.0 or item == float('Inf') or np.isinf(item):
                    scan_range.append(6.0-min_range)
                else:
        ...
        scan_array_np = np.array(scan_range)
        min_pooled_array = scan_array_np.reshape(21, -1).min(axis=1)
        scan_range = min_pooled_array.tolist()
        ...
        constant = (6.0-min_range)*np.ones(21)
        ...
        #     if i != 10.0 or i >= 0.02:
        #         laser_input[t] = laser_input[t] - 0.02
        #     t +=1
    ...
    def setReward(self, state, done, action):
        ...
        self.total_path_length  += path_lenght
        # print('total_path_lenght:' + str(self.total_path_length))
    ...
    def goal_def(self, goal_index):
        ...
        #     x,y = 4.5, 0.0
        #     # x,y = 5.0, -6.0
        #     # x, y =  8.0, np.random.randint(-86,86)/10
        # elif location == 1:
        #     x,y = 4.5, 4.0
        if location == 0: 
            # x,y = -7.87, 5.62
    ...
    def reset(self, done):
        ...
        # self.goal_x, self.goal_y = 5.6,-8.8
        # time.sleep(0.1)
